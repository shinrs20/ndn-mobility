...
namespace ns3 {
  namespace nnn {
	 ...
	 */
	Entry (Pit &container, Ptr<const ndn::Interest> header, Ptr<fib::Entry> fibEntry);
	...
	Ptr<const ndn::Interest> m_interest; ///< \brief Interest of the PIT entry (if several interests are received, then nonce is from the first Interest)
